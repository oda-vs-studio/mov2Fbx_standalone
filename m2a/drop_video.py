...
def publish_result(run,report,dest):
    """Publish existing FBX results without running inference."""
    ...
    return [target for _,target in files]
...
def main():
    ...
    if args.export_run:
        import json
        run=args.export_run.resolve()
        report=json.loads((run/'segments_manifest.json').read_text(encoding='utf-8'))
        folder=run.parent/('Recovered_'+uuid.uuid4().hex[:8])
        folder.mkdir()
        publish_result(run,report,folder/'scene.fbx')
        return 0
    if not args.videos:parser.error('Provide a video or --export-run')
    failed=False
    for video in args.videos:
        ...
        try:
            ...
            short_stem=video.stem.encode('utf-16-le')[:48].decode('utf-16-le',errors='ignore').rstrip(' .')
            result_dir=video.parent/(short_stem+'_Movie2Anim_'+token) if args.output_folder else video.parent
            dest=result_dir/('scene.fbx' if args.output_folder else video.stem+'.fbx')
            if args.output_folder and len(str(result_dir/'_work/segments/001/person_01/quinn_retarget.json').encode('utf-16-le'))//2>=250:
                raise ValueError('Parent directory is too long for native FBX tools. Move the input video to a shorter directory before processing.')
            ...
            if args.output_folder:
                result_dir.mkdir()
                (result_dir/'input.json').write_text(__import__('json').dumps({'video':str(video)},ensure_ascii=False,indent=2),encoding='utf-8')
