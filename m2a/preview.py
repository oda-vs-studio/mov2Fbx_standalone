...
def write_preview(path, joints, fps, metadata):
    payload = json.dumps(dict(joints=joints.round(5).tolist(),parents=PARENTS,fps=fps,source_start=metadata['start_frame']),separators=(',',':'))
...
let yaw=.35,pitch=.13,zoom=1,playing=false,start=0,drag=null;const xs=data.joints.flat().map(p=>p[0]),zs=data.joints.flat().map(p=>p[2]);const cx=(Math.min(...xs)+Math.max(...xs))/2,cz=(Math.min(...zs)+Math.max(...zs))/2;
...
function draw(t){if(playing)seek.value=Math.floor((t-start)/1000*data.fps)%data.joints.length;let i=Number(seek.value);ctx.clearRect(0,0,c.width,c.height);for(let k=-5;k<=5;k++){line([cx+k*.5,0,cz-2.5],[cx+k*.5,0,cz+2.5],'#273448');line([cx-2.5,0,cz+k*.5],[cx+2.5,0,cz+k*.5],'#273448');}const j=data.joints[i];for(let b=1;b<j.length;b++)line(j[b],j[data.parents[b]],b<25?'#69befa':'#5c7ea6',b<25?3:1);for(let b=0;b<22;b++){let p=project(j[b]);ctx.beginPath();ctx.arc(...p,3*devicePixelRatio,0,Math.PI*2);ctx.fillStyle='#eaf6ff';ctx.fill();}document.getElementById('frame').textContent=`${i+1} / ${jointsCount()} · source ${data.source_start+i}F`;requestAnimationFrame(draw);}
