...
`DropVideoToMovie2Anim.bat` passes `--actor-mode primary`; the CLI and `solve_auto` otherwise retain `all` as their default. Primary selection ranks tracks by actual observed frame count, then median observed box area, and keeps one existing ID for the whole clip. All detection/tracking artifacts remain intact and camera estimation completes using every tracked person before body-subject selection. Only selected subjects enter interval partitioning and body inference. Actor selection is recorded in `actor_selection.json` and exported reports. No cross-ID reidentification or long-gap pose synthesis is added. `--actor-mode all` restores all-actor interval handling.
...
SetupMovie2Anim.bat invokes tools.setup_all under Python 3.13 x64, which runs tools.bootstrap_setup followed by tools.setup_camera. Interactive setup selects the UE folder with tkinter; the bundled Quinn skeleton removes the need for another FBX selection. The setup detects MSVC using vswhere, creates/reuses .venv, installs pinned requirements, stages and validates UE model extraction, builds all three helpers, downloads the pinned YOLOX model, then runs tests and doctor. Existing complete models are hash-checked and reused; differing existing model files are not silently overwritten. Extraction staging and explicit Quinn replacement backups stay under ignored work/. See README for prerequisites, exact source/destination paths and noninteractive flags.

## Experimental moving camera

SetupMovie2Anim.bat isolates Python 3.12 / Torch 2.7.1 cu128 / NumPy 1.26 from the existing body environment. tools.camera_worker runs as a subprocess and releases CUDA allocations on exit before the ONNX stages. Sources are pinned official repositories; minimal inference dependencies use Torch SDPA without custom CUDA compilation.

GeoCalib shared simple-radial calibration rectifies the entire fixed-lens clip once. Tracking, DA3 and body inference consume that identical FFV1 raster. DA3 windows cover every frame and share overlap depth geometry. Background PnP uses the fixed rectified intrinsics and masks every detected person for checks; the DA3 network itself remains unmasked. Sequential fixed-K PnP uses DA3 depth and a DA3 pose seed. Overlap depth scale plus the exact last shared camera pose defines a Sim(3), aligning both depths and w2c extrinsics; camera centers are -R.T t. Float32 network rotations are projected onto SO(3) before skeletal operations.

m2a.moving_camera.world_actors aligns camera scale to pooled torso/body depth estimates from all actors. Raw Hue translations plus the shape-dependent pelvis offset are treated as camera-relative; inverse DA3 poses transform only the root rotation/position to the shared world. GeoCalib camera-up samples establish world up, then existing common floor and target scale are applied. Child-local rotations and raw motion.npz remain unchanged. CamAngvel remains zero because its trained encoding is unavailable; this is an explicit approximation, not a native moving-camera Hue implementation. See MOVING_CAMERA.md and MOVING_CAMERA_PLAN.md for diagnostics and limits.
