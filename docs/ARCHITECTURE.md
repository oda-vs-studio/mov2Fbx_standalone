...
SetupCamera.bat isolates Python 3.12 / Torch 2.7.1 cu128 / NumPy 1.26 from the existing body environment. tools.camera_worker runs as a subprocess and releases CUDA allocations on exit before the ONNX stages. Sources are pinned official repositories; minimal inference dependencies use Torch SDPA without custom CUDA compilation.
