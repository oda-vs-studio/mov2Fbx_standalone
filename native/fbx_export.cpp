...
int wmain(int argc, wchar_t** argv) {
    try {
        ...
        if (!(in >> bones >> frames >> fps) || bones < 1 || bones > 256 || frames < 1 || frames > 100000 || !(fps > 0 && fps < 1000)) throw std::runtime_error("bad header");
        ...
        for (int i=0;i<bones;i++) {
            std::string name; int parent; double x,y,z;
            if (!(in >> name >> parent >> x >> y >> z) || parent >= i || parent < -1 || (i>0 && parent<0)) throw std::runtime_error("bad skeleton");
            ...
            node->LclTranslation.Set(i==0?FbxDouble3(0,0,0):FbxDouble3(x,y,z));
        ...
        for(int f=0;f<frames;f++) {
            ...
            for(int k=0;k<3+bones*3;k++) {
                double value;
                if(!(in>>value) || !std::isfinite(value)) throw std::runtime_error("bad animation key");
                original.push_back(value);
                FbxAnimCurve* curve = k<3 ? nodes[0]->LclTranslation.GetCurve(layer,channels[k],true) : nodes[(k-3)/3]->LclRotation.GetCurve(layer,channels[(k-3)%3],true);
